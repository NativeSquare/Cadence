...
**Goal**:
The Athlete's single active target. Holds a *category* (only `"race"` at MVP), a *raceTarget* (e.g. `{ type: "time", seconds: 11940 }` or a finish target), and a *raceId* pointing at the **Race**. (The general-fitness category is **removed from the domain model** at MVP — so every Goal is race-anchored. It survives only as a disabled "coming soon" teaser card in the onboarding and change-goal chooser step, which creates no Goal and drives no flow — see ADR-0008.) An Athlete has exactly **one active Goal**; because every Goal references a Race, every Goal has a date. A race Goal's `status` is **derived from its Race's status**, never set directly.
