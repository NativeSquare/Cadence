...
function athleteToForm(
  athlete: { sex?: Sex; dateOfBirth?: string; weightKg?: number; heightCm?: number; maxHr?: number; restingHr?: number } | null | undefined,
): FormState {
  if (!athlete) return EMPTY_FORM;
  ...
  return {
    ...
    maxHr: athlete.maxHr != null ? String(athlete.maxHr) : "",
    restingHr: athlete.restingHr != null ? String(athlete.restingHr) : "",
  };
}
...
export default function AthleteScreen() {
  ...
  React.useEffect(() => {
    if (athlete === undefined || hydrated) return;
    const next = athleteToForm(athlete);
    setInitial(next);
    setForm(next);
    setHydrated(true);
  }, [athlete, hydrated]);

  const hasChanges = !isFormEqual(form, initial);
  ...
  const handleSave = async () => {
    ...
    const dobProvided =
      form.dobDay.length > 0 || form.dobMonth.length > 0 || form.dobYear.length > 0;
    ...
    const candidate = {
      ...
      maxHr: parseNumber(form.maxHr),
      restingHr: parseNumber(form.restingHr),
    };

    const parsed = AthleteProfileSchema.safeParse(candidate);
    if (!parsed.success) {
    ...
    setIsLoading(true);
    try {
      await upsertAthlete(parsed.data);
      router.back();
    } catch (err) {
