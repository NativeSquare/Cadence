...
- [ ] Task 1: Create DebriefScreen container (AC: #1)
  - [ ] Create `apps/native/src/components/app/session/DebriefScreen.tsx`
  - [ ] Implement phase state management (0 → 1 → 2 → submitted → coachReply)
  - [ ] Add scroll behavior with ref
  - [ ] Implement `debriefIn` entrance animation

- [ ] Task 2: Create DebriefHeader component (AC: #1)
  - [ ] Create `DebriefHeader.tsx` with completion badge and title
  - [ ] Implement stats row with Time, Distance, Avg Pace cards
  - [ ] Add `springUp` staggered animation for stats cards

- [ ] Task 3: Create FeelingSelector component (AC: #2)
  - [ ] Create `FeelingSelector.tsx` with 5 feeling options
  - [ ] Use exact FEELING_OPTIONS constant values
  - [ ] Implement selection state with checkmark animation (`checkPop`)
  - [ ] Add staggered `springUp` animation per option

- [ ] Task 4: Create QuickTagPills component (AC: #3)
  - [ ] Create `QuickTagPills.tsx` with flex-wrap layout
  - [ ] Use exact DEBRIEF_PILLS constant values
  - [ ] Implement toggle selection (dashed → solid border)

- [ ] Task 5: Create DebriefNoteInput component (AC: #3, #4)
  - [ ] Create `DebriefNoteInput.tsx` with textarea
  - [ ] Implement mic button and character count
  - [ ] Add send arrow button animation (`scaleIn`)

- [ ] Task 6: Create VoiceRecorderMode component (AC: #4)
  - [ ] Create `VoiceRecorderMode.tsx` with waveform visualization
  - [ ] Implement 24-bar animated waveform (`waveform` keyframes)
  - [ ] Add timer display and Cancel/Done buttons

- [ ] Task 7: Create useStreamingText hook (AC: #5)
  - [ ] Create `apps/native/src/hooks/use-streaming-text.ts`
  - [ ] Implement character-by-character streaming (22ms speed, 300ms delay)
  - [ ] Return `{ displayed, done, started }` state

- [ ] Task 8: Create CoachResponseCard component (AC: #5)
  - [ ] Create `CoachResponseCard.tsx` with dark background card
  - [ ] Implement coach avatar (lime circle with "C")
  - [ ] Use streaming text hook with blinking cursor
  - [ ] Add radial gradient decoration

- [ ] Task 9: Create DebriefSummary component (AC: #6)
  - [ ] Create `DebriefSummary.tsx` with "Logged" indicator
  - [ ] Display selected feeling, pills, and note indicator
  - [ ] Add "Done" button with springUp animation

- [ ] Task 10: Create CelebrationOverlay component (AC: #7)
  - [ ] Create `CelebrationOverlay.tsx` with phase-based animations
  - [ ] Implement check circle with glow (`celebCheck`)
  - [ ] Add SVG ring burst animations (`celebRing`, `celebRing2`)
  - [ ] Implement text reveal and auto-dismiss (2800ms)

- [ ] Task 11: Wire navigation and data flow
  - [ ] Accept props from Active Session: `{ session, elapsedTime, distanceCovered }`
  - [ ] Implement `onDone` callback for navigation back to Plan screen
  - [ ] Mark session as complete in state

- [ ] Task 12: Add to component exports and verify integration
  - [ ] Export all components from `apps/native/src/components/app/session/index.ts`
  - [ ] Test full flow from mocked Active Session
