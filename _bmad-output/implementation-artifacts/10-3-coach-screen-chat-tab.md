...
- [ ] Task 1: Create CoachScreen container (AC: #1)
  - [ ] Create `apps/native/src/app/(app)/coach/index.tsx` screen route
  - [ ] Create `apps/native/src/components/app/coach/CoachScreen.tsx` main component
  - [ ] Implement scroll container with keyboard-aware behavior
  - [ ] Add safe area handling for iOS notch/home indicator

- [ ] Task 2: Build chat header component (AC: #1)
  - [ ] Create `apps/native/src/components/app/coach/ChatHeader.tsx`
  - [ ] Display "Coach" title with proper typography
  - [ ] Implement online/typing status indicator with animated dot
  - [ ] Match design: dark header with lime accent, "Context" button pill

- [ ] Task 3: Build chat message components (AC: #1, #3)
  - [ ] Create `apps/native/src/components/app/coach/ChatMessage.tsx`
  - [ ] Implement coach message bubble (left-aligned, white bg, rounded corners)
  - [ ] Implement user message bubble (right-aligned, dark bg, lime accents)
  - [ ] Add coach label badge with status dot for coach messages
  - [ ] Implement entry animation (`msgIn` keyframe from prototype)

- [ ] Task 4: Create ToolCard component for tool results (AC: #2)
  - [ ] Create `apps/native/src/components/app/coach/ToolCard.tsx`
  - [ ] Implement lime border styling with title bar
  - [ ] Create TrainingLoadCard variant showing Acute/Chronic/Ratio
  - [ ] Add warning/caution styling for ratio values
  - [ ] Connect to existing tool-renderer pattern for extensibility

- [ ] Task 5: Implement typing indicator (AC: #3)
  - [ ] Create `apps/native/src/components/app/coach/TypingIndicator.tsx`
  - [ ] Implement 3-dot animation matching prototype (`typingDot` keyframe)
  - [ ] Show/hide based on streaming state from useAIChat

- [ ] Task 6: Build ChatInput component (AC: #1, #3)
  - [ ] Create `apps/native/src/components/app/coach/ChatInput.tsx`
  - [ ] Implement text input with placeholder "Ask your coach..."
  - [ ] Add mic button (transitions to recording mode)
  - [ ] Add send button with enabled/disabled states
  - [ ] Handle keyboard events (Enter to send)
  - [ ] Match light theme styling from prototype (white input, border)

- [ ] Task 7: Implement VoiceRecorder mode (AC: #4, #5)
  - [ ] Create `apps/native/src/components/app/coach/VoiceRecorder.tsx`
  - [ ] Reuse existing `useVoiceInput` hook from onboarding
  - [ ] Implement waveform visualization (20 animated bars per prototype)
  - [ ] Show live transcription above input area
  - [ ] Add cancel (X) and send buttons in recording mode
  - [ ] Integrate with existing VoiceInput component patterns

- [ ] Task 8: Connect to AI streaming infrastructure (AC: #3)
  - [ ] Wire up `useAIChat` hook for message state management
  - [ ] Configure conversation persistence
  - [ ] Handle tool call/result rendering via MessageParts pattern
  - [ ] Implement auto-scroll on new messages

- [ ] Task 9: Add mock data mode for development
  - [ ] Create mock conversation data matching prototype (INIT_MSGS)
  - [ ] Create mock replies array for testing without AI backend
  - [ ] Add mock typing delay simulation
