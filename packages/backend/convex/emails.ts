...
/** Wraps body HTML in the branded Cadence email shell. */
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f3f3f3;">
    <tr><td align="center" style="padding:40px 16px;">
      <table role="presentation" width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;">
        ...
        <!-- Footer -->
        <tr><td align="center" style="padding-top:32px;">
          <p style="margin:0;font-size:11px;color:#b4b4b4;line-height:1.6;">&copy; 2026 ${APP_NAME} &middot; NativeSquare SAS &middot; 60 rue Fran&ccedil;ois 1er, 75008 Paris</p>
          ${unsubscribeUrl ? `<p style="margin:8px 0 0;font-size:11px;"><a href="${unsubscribeUrl}" style="color:#b4b4b4;text-decoration:underline;">Unsubscribe</a></p>` : ""}
        </td></tr>
...
export const sendBroadcastEmail = internalAction({
  ...
  handler: async (ctx, args) => {
    const isDev = process.env.IS_DEV === "true";
    if (isDev) {
      console.log(`[DEV] Broadcast email to ${args.to}: "${args.subject}"`);
      return "dev-email-id";
    }

    const html = buildBroadcastHtml(args.bodyHtml);
